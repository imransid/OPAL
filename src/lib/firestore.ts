...
export async function getProducts(): Promise<Product[]> {
  ...
  try {
    ...
    console.warn('[opal] getProducts: ordered query failed, fetching without orderBy', e);
    try {
      const snap = await getDocs(collection(db, PRODUCTS));
      const list = snap.docs.map((d) => normalizeProduct(d.data() as Record<string, unknown>, d.id));
      return list.sort((a, b) => (a.title ?? '').localeCompare(b.title ?? '', undefined, { sensitivity: 'base' }));
    } catch (e2) {
      console.error('[opal] getProducts:', e2);
      return [];
    }
  }
}
...
export async function getProduct(id: string): Promise<Product | null> {
  ...
  try {
    ...
    console.error('[opal] getProduct:', e);
    return null;
  }
}
...
export async function getCategories(): Promise<Category[]> {
  ...
  try {
    ...
    console.warn('[opal] getCategories: ordered query failed, fetching without orderBy', e);
    try {
      const snap = await getDocs(collection(db, CATEGORIES));
      const list = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Category));
      return list.sort((a, b) => (a.title ?? '').localeCompare(b.title ?? '', undefined, { sensitivity: 'base' }));
    } catch (e2) {
      console.error('[opal] getCategories:', e2);
      return [];
    }
  }
}
...
export async function getStoreSettings(): Promise<StoreSettings> {
  ...
  try {
    ...
    console.error('[opal] getStoreSettings:', e);
    return {};
  }
}
...
export async function createOrder(data: CreateOrderInput): Promise<{ id: string; orderNumber: string }> {
  ...
  const items = data.items.map((item) => omitUndefined(item as unknown as Record<string, unknown>));
